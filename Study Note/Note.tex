...
\end{itemize}
\newpage
\subsection*{Continous Dostribution}
\begin{tabular}{|l|l|c|c|}
    \hline
    $\theta \sim$Distrubution & Density function & Mean & Variance\\
    \hline\hline
    Uniform$[a,b]$ & $\displaystyle f(\theta) = \frac{1}{b-a}$ & $\displaystyle\frac{a+b}{2}$ & $\displaystyle \frac{(b-a)^2}{12}$\\
    \hline
    Normal$(\mu,\sigma^2)$ & $\displaystyle f(\theta) = \frac{\exp{\left(-\frac{(\theta - \mu)^2}{2\mu^2}\right)}}{\sqrt{2 \pi \sigma^2}}$ & $\mu$ & $\sigma^2$\\
    \hline
    log Normal$(\mu,\sigma^2)$ & $\displaystyle f(\theta) = \frac{\exp{\left(-\frac{(\log\theta - \mu)^2}{2\mu^2}\right)}}{\theta\sqrt{2 \pi \sigma^2}}$ & $\displaystyle\exp\left(\mu + \frac{\sigma^2}{2}\right)$ & $\displaystyle\exp(2\mu + \sigma^2) (\exp(\sigma^2) -1)$\\
    \hline
    $\beta(\alpha,\beta)$ & $\displaystyle f(\theta) = \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha)\Gamma(\beta)} \theta^{\alpha-1}(1-\theta)^{\beta-1}$ & $\displaystyle \frac{\alpha}{\alpha +\beta}$ & $\displaystyle\frac{\alpha\beta}{(\alpha + \beta)^2 (\alpha + \beta +1)}$\\
    \hline
    Exponential$(\lambda)$ & $\displaystyle f(\theta) = \lambda \exp(-\lambda\theta)$ & $\displaystyle \frac{1}{\lambda}$ & $\displaystyle \frac{1}{\lambda^2}$\\
    \hline
    $\Gamma(\alpha,\beta)$ & $\displaystyle f(\theta) = \frac{\beta^\alpha}{\Gamma(\alpha)}\theta^{\alpha -1}\exp(-\beta\theta)$ & $\displaystyle \frac{\alpha}{\beta}$ & $\displaystyle \frac{\alpha}{\beta^2}$\\
    \hline
    $\Gamma^{-1}(\alpha,\beta)$ & $\displaystyle f(\theta) = \frac{\beta^\alpha}{\Gamma(\alpha)}\theta^{-(\alpha+1)}\exp\left(-\frac{\beta}{\theta}\right)$ & $\displaystyle\frac{\beta}{\alpha -1}$ & $\displaystyle\frac{\beta^2}{(\alpha -1)^2(\alpha-2)}$\\
    \hline
    $\chi_v^2$ & $\displaystyle f(\theta) = \frac{2^{-v/2}}{\Gamma(v/2)} \theta^{\frac{v}{2}-1} \exp\left(-\frac{\theta}{2}\right)$ & $v$ & $2v$\\
    \hline
    $\chi_v^{-2}$ & $\displaystyle f(\theta) = \frac{2^{-v/2}}{\Gamma(v/2)} \theta^{-\left(\frac{v}{2}+1\right)} \exp\left(-\frac{1}{2\theta}\right)$ & $\displaystyle\frac{1}{v-2}$ & $\displaystyle\frac{2}{(v-2)^2(v-4)}$\\
    \hline
    Dirichlet$(\alpha_1,...\alpha_k)$ & $\displaystyle f(\theta) = \frac{\Gamma(\alpha_0)}{\prod_{i=1}^k \Gamma(\alpha_i)}\prod_{i=1}^k \theta_i^{\alpha_i -1}$ & $\displaystyle \Expect{\theta_i} = \frac{\alpha_i}{\alpha_0}$ & $\Var{\theta_i} = \displaystyle\frac{\alpha_i(\alpha_0 - \alpha_i)}{\alpha_0^2 (\alpha_0 + 1)}$\\
    \hline
\end{tabular}
\\~\\Noted that for Dirichlet distribution, $\displaystyle \alpha_0 = \sum_{i=1}^k \alpha_i $
